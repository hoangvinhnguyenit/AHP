...
def load_data():
    try:
        ...
        st.error(f"❌ Lỗi nạp dữ liệu nhà: {e}")
        return None
...
if df_raw is not None:
    ...
    if not df_l1.empty:
        ...
        if not st.session_state.registered:
            ...
            with tab2:
                cols = st.columns(3)
                for i, (col, row) in enumerate(zip(cols, df_ranked.head(3).iterrows())):
                    data = row[1]
                    with col:
                        st.markdown(f"<div style='border:2px solid #FF4B4B; border-radius:10px; padding:10px; text-align:center;'><b>TOP {i+1}: {data['id']}</b></div>", unsafe_allow_html=True)
                        st.write(f"📍**Khu vực:**  {data['ten_quan']} | 💰  **Giá:**{data['gia_ban']} tỷ")
                        st.write(f"📐 **Loại bất động sản:** {data['loai_bds']} m2")
                        st.write(f"📐**Diện tích:**  {data['dien_tich']} m2 | 📄 **Pháp lý:** {data['giay_to']}")
                        st.write(f"🛣️ Đường: {data['duong']} | ✨**Đặc điểm:** {data['dac_diem']}")
                        st.write(f"📄 **Tiện ích:** {data['tien_ich']}")
                        st.write(f"💰 **Vị trí:** {data['vi_tri']} km")
                        st.write(f"🧭 **Hướng nhà:** {data['huong_nha']}")
